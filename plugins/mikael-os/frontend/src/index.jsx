...
// Immediate PDF lane: multipart upload -> request-scoped temp files -> local
// Poppler extraction -> cited multi-PDF manifests. No model/provider call and
// no durable, Qdrant, Neo4j, Anki, mission, or business write.
...
function sdkPostForm(url, form) {
  const sdk = (typeof window !== "undefined" && window.__HERMES_PLUGIN_SDK__) || {};
  if (typeof sdk.authedFetch === "function") {
    return Promise.resolve(sdk.authedFetch(url, { method: "POST", body: form }))
      .then((r) => (r && typeof r.json === "function" ? r.json().then((body) => {
...
function CoachIntake(props) {
  ...
  const submit = useCallback((event) => {
    ...
    sdkPostForm(LEARNING_INTAKE_API, form)
      .then((data) => { setResult(data || null); setPhase("done"); })
      .catch((reason) => {
  ...
  return h(
    "div", { className: "mos__co-scroll" },
    h("div", { className: "mos__co-plan-intro" }, h(Icon, { name: "file-plus", size: 14 }),
      h("span", null, "Bis zu 8 PDFs werden sofort lokal gelesen und mit Seitenzitaten in den Kontext gelegt. ",
        h("b", null, "Nur Studium/privat"), " · kein Upload-Speicher, kein Embedding, kein Graph-Write.")),
    h("form", { className: "mos__intake-form", onSubmit: submit },
      h("label", { className: "mos__intake-drop" },
        ...
        h("input", { type: "file", accept: "application/pdf,.pdf", multiple: true,
          onChange: (e) => setFiles(Array.from((e.target && e.target.files) || []).slice(0, 8)) })),
      h("div", { className: "mos__intake-grid" },
      ...
      h("label", { className: "mos__intake-question" }, h("span", null, "Was soll Jarvis daraus beantworten?"),
      ...
      h("button", { type: "submit", className: "mos__co-btn mos__co-btn--primary", disabled: phase === "loading" },
    ...
    result ? h("div", { className: "mos__intake-receipt mos--" + (dc.answer_ready ? "verified" : "amber") },
      h("div", { className: "mos__intake-receipt-head" }, h(Icon, { name: dc.answer_ready ? "circle-check-big" : "triangle-alert", size: 16 }),
        h("b", null, dc.answer_ready ? "Antwortbereit" : "Partitionsauswahl nötig"),
        h("code", null, receipt.receipt_id || "—")),
      h("div", { className: "mos__intake-stats" },
      ...
      h("div", { className: "mos__co-honest" }, h(Icon, { name: "lock", size: 12 }),
        h("span", null, "Receipt ist deterministisch, aber nicht gespeichert. will_write=false · Qdrant=false · Neo4j=false."))) : null,
  );
}
