...
from learning_intake.attachments import (  # noqa: E402
    PdfAttachment,
...
def _attachment_runner(command: list[str], _timeout: float) -> subprocess.CompletedProcess[str]:
    if command[0] == "pdfinfo":
        return subprocess.CompletedProcess(command, 0, "Pages: 1\n", "")
    if command[0] == "pdftotext":
        source = Path(command[-2])
        label = "Alpha Inhalt" if b"alpha" in source.read_bytes() else "Beta Inhalt"
        return subprocess.CompletedProcess(command, 0, label + "\f", "")
    raise AssertionError(f"unexpected command: {command}")


def test_multi_pdf_bundle_is_cited_deduplicated_and_receipted(tmp_path: Path) -> None:
    alpha = tmp_path / "alpha.pdf"
    alpha_copy = tmp_path / "alpha-kopie.pdf"
    beta = tmp_path / "beta.pdf"
    alpha.write_bytes(b"%PDF-1.4 alpha")
    alpha_copy.write_bytes(alpha.read_bytes())
    beta.write_bytes(b"%PDF-1.4 beta")
    attachments = gateway_pdf_attachments(
        [alpha, beta, alpha_copy],
        media_types=["application/pdf"] * 3,
    )
    kwargs = {
        "tenant_id": "uni:tum",
        "module_id": "thermodynamik-1",
        "exam_id": "thermodynamik-ws26",
        "exam_date": "2026-12-18",
        "question": "Vergleiche Alpha und Beta.",
        "observed_at": "2026-07-19T12:00:00Z",
        "runner": _attachment_runner,
    }

    first = analyze_pdf_attachments(attachments, **kwargs)
    second = analyze_pdf_attachments(attachments, **kwargs)

    assert first["receipt"] == second["receipt"]
    assert first["receipt"]["will_write"] is False
    assert first["receipt"]["persisted"] is False
    assert first["receipt"]["writes"] == []
    direct = first["direct_context"]
    assert direct["answer_ready"] is True
    assert direct["document_count"] == 2
    assert direct["input_count"] == 3
    assert direct["duplicate_count"] == 1
    assert direct["embedding_requested"] is False
    assert direct["graph_write_requested"] is False
    assert direct["durable_write_requested"] is False
    assert len(direct["citations"]) == 2
    assert "Alpha Inhalt" in direct["context_text"]
    assert "Beta Inhalt" in direct["context_text"]
    assert first["attachments"][2]["duplicate"] is True
    assert first["attachments"][2]["receipt_id"] == first["attachments"][0]["receipt_id"]
    assert all(
        document["source"]["intake_path"].startswith("/direct-context/")
        for document in first["documents"]
    )


def test_multi_pdf_context_never_exceeds_bundle_budget(tmp_path: Path) -> None:
    first = tmp_path / "bounded.pdf"
    first.write_bytes(b"%PDF-1.4 bounded")
    result = analyze_pdf_attachments(
        [PdfAttachment(first, "bounded.pdf")],
        tenant_id="uni:mikael",
        module_id="recht",
        exam_id="klausur-1",
    ...
    assert len(context["context_text"]) == 20
    assert context["context_text_chars"] == 20
    assert context["context_truncated"] is True
    assert context["answer_ready"] is False
    assert context["partition_required"] is True


def test_multi_pdf_receipts_remain_tenant_scoped(tmp_path: Path) -> None:
    source = tmp_path / "skript.pdf"
    source.write_bytes(b"%PDF-1.4 alpha")
    attachment = PdfAttachment(source, "skript.pdf")
    common = {
        "module_id": "mechanik-1",
        "exam_id": "mechanik-2026",
        "exam_date": "2026-08-25",
        "question": "Was ist wichtig?",
        "observed_at": "2026-07-19T12:00:00Z",
    ...
    tum = analyze_pdf_attachments([attachment], tenant_id="uni:tum", **common)
    lmu = analyze_pdf_attachments([attachment], tenant_id="uni:lmu", **common)

    assert tum["receipt"]["receipt_id"] != lmu["receipt"]["receipt_id"]
    assert tum["documents"][0]["dedup_key"] != lmu["documents"][0]["dedup_key"]


def test_gateway_attachment_adapter_rejects_non_pdf_media(tmp_path: Path) -> None:
    source = tmp_path / "notizen.pdf"
    source.write_bytes(b"%PDF-1.4 alpha")
    with pytest.raises(DirectContextError, match="media type"):
        gateway_pdf_attachments([source], media_types=["text/plain"])
